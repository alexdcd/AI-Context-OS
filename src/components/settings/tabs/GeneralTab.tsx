...
import { VaultSettingsSection } from "../../vault/VaultSettingsSection";
...
export function GeneralTab() {
  ...
  const handleRestore = useCallback(async () => {
    const result = await open({ filters: [{ name: "Zip", extensions: ["zip"] }], multiple: false });
    if (!result) return;
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Language */}
      <section className="obs-panel border border-[color:var(--border)] p-6">
        <h2 className="mb-4 text-lg font-medium text-[color:var(--text-0)]">{t("settings.language.label")}</h2>
      ...
      <section className="obs-panel border border-[color:var(--border)] p-6">
        ...
        <p className="mb-4 text-sm text-[color:var(--text-2)]">{t("settings.explorer.expertModeDesc")}</p>
        <button
            ...
            <div className="flex items-center justify-between gap-3">
              <div>
                ...
                <p className="mt-2 text-sm text-[color:var(--text-2)]">
                  {expertModeEnabled ? t("settings.explorer.expertModeActive") : t("settings.explorer.showSystemFilesDesc")}
                </p>
              </div>
              <span className={clsx("relative inline-flex h-6 w-11 shrink-0 items-center rounded-full transition-colors", expertModeEnabled ? "bg-[color:var(--accent)]" : "bg-[color:var(--bg-3)]")}>
                <span className={clsx("inline-block h-4 w-4 rounded-full bg-white transition-transform", expertModeEnabled ? "translate-x-6" : "translate-x-1")} />
              </span>
            </div>
          </div>
        </button>
        {expertModeEnabled && (
          ...
          >
            <div className="mt-0.5">
              {showSystemFiles ? <Eye className="h-5 w-5 text-[color:var(--accent)]" /> : <EyeOff className="h-5 w-5 text-[color:var(--text-2)]" />}
            </div>
            <div className="flex-1">
              <div className="flex items-center justify-between gap-3">
                <span className="font-medium text-[color:var(--text-0)]">{t("settings.explorer.showSystemFiles")}</span>
                <span className={clsx("rounded-full px-2 py-0.5 text-[11px] font-medium", showSystemFiles ? "bg-[color:var(--accent)] text-white" : "bg-[color:var(--bg-2)] text-[color:var(--text-2)]")}>
                  {showSystemFiles ? t("settings.explorer.showSystemFilesActive") : t("settings.explorer.showSystemFilesHidden")}
                </span>
              </div>
              <p className="mt-2 text-sm text-[color:var(--text-2)]">
                {showSystemFiles ? t("settings.explorer.showSystemFilesVisible") : t("settings.explorer.showSystemFilesHiddenDesc")}
              </p>
            </div>
          </button>
        )}
      ...
      <section className="obs-panel border border-[color:var(--border)] p-6">
        ...
        <p className="mb-4 text-sm text-[color:var(--text-2)]">{t("settings.backup.desc")}</p>
        <div className="flex flex-col gap-3">
          <button
          ...
          >
            {backupStatus === "loading" ? <Loader2 className="h-5 w-5 animate-spin text-[color:var(--accent)]" /> : backupStatus === "done" ? <Check className="h-5 w-5 text-[color:var(--success)]" /> : <Download className="h-5 w-5 text-[color:var(--text-1)]" />}
            <div>
              ...
              <p className="mt-0.5 text-sm text-[color:var(--text-2)]">
                {backupStatus === "done" ? t("settings.backup.exportSuccess") : backupStatus === "error" ? t("settings.backup.exportError") : t("settings.backup.exportDesc")}
              </p>
            </div>
          </button>
          <button
            ...
            className="flex items-center gap-3 rounded-md border border-[color:var(--border)] bg-[color:var(--bg-0)] p-4 text-left transition-colors hover:border-[color:var(--border-active)]"
          >
            {restoreStatus === "loading" ? <Loader2 className="h-5 w-5 animate-spin text-[color:var(--accent)]" /> : restoreStatus === "done" ? <Check className="h-5 w-5 text-[color:var(--success)]" /> : <Upload className="h-5 w-5 text-[color:var(--text-1)]" />}
            <div>
              <span className="font-medium text-[color:var(--text-1)]">{t("settings.backup.restore")}</span>
              <p className="mt-0.5 text-sm text-[color:var(--text-2)]">
                {restoreStatus === "done" ? t("settings.backup.restoreSuccess") : restoreStatus === "error" ? t("settings.backup.restoreError") : t("settings.backup.restoreDesc")}
              </p>
            </div>
          </button>
        </div>
